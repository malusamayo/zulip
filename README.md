...
* *Typing auto-complete feature:* Zulip can also use an LLM to provide context-aware typing auto-complete, suggesting message continuations as the user types. This feature should be triggered when users type, with continuations rendered in the text box. When implementing this feature, you will want to consider the associated cost and latency, and choose appropriate models and time to trigger this feature.
...
* 15p: The report describes how message auto-complete is implemented and we can find the corresponding implementation. An LLM was used in the implementation. The application is functional in that it generates auto-complete when users type. A video is recorded showing how a user will use the feature in the user interface.
